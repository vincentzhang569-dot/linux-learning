...
if "messages" not in st.session_state:
    st.session_state.messages = [
        {
            "role": "system",
            "content": """你是一台"工业故障诊断终端"，不是聊天助手。

你的唯一任务是输出技术解决方案。

【严格遵守以下规则】：

1. 禁止输出任何客套话（如"您好"、"请随时提问"、"好的"等）。

2. 当用户提及某个故障名词（如"伺服电机故障"），必须立即列出：

   - 可能的故障原因（1,2,3...）

   - 对应的排查步骤（1,2,3...）

3. 格式必须是清晰的 Markdown 列表。

4. 如果信息不足，优先给出最常见情况的通解，而不是反问用户。"""
        }
    ]
...
QUICK_PROMPT_MAPPING = {
    "查伺服电机故障": "请详细列出伺服电机常见故障的原因和排查步骤",
    "查通讯超时": "PLC与上位机通讯超时的常见原因和解决办法是什么？",
    "ABB 机器人错误代码大全": "请列出ABB机器人常见的错误代码及其含义和解决方法",
    "编码器故障排查": "编码器故障的常见原因和排查步骤是什么？",
    "PLC 通讯异常": "PLC通讯异常的常见原因和排查方法是什么？"
}
        ...
        with st.chat_message("assistant"):
            ...
            with st.spinner(spinner_text):
                ...
                stream = client.chat.completions.create(
                    ...
                    messages=messages,
                    stream=True,
                    temperature=0.1  # 降温：强制模型冷静、客观、直接输出
                )
